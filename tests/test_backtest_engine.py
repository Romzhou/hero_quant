...
def test_align_multi_asset_returns_series():
    ...
    import numpy as np
    res = engine.run(prices, weights=np.array([0.9, 0.1]), costs=0.0)
    res2 = engine.run(prices, weights=np.array([0.1, 0.9]), costs=0.0)
...
def test_pit_guard_default_on():
    ...
    res = engine.run(prices, weights=[1.0])
    assert "equity" in res
...
def test_leverage_isclose_and_bear_not_overridden():
    ...
    res_bear = engine.run(prices, weights=np.zeros(2), costs=0.0)
    ...
    res_tiny = engine.run(prices, weights=w_tiny, costs=0.0)
...
def test_output_dir_io_failure_surfaces(tmp_path):
    ...
    with pytest.raises(Exception):
        engine.run(prices, weights=[1.0], output_dir=file_path)
